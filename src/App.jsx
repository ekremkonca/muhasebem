...
export default function App(){
  ...
  useEffect(()=>{
    ...
    loadRecords()
      ...
      .catch(err=>{if(mounted)setError(err.message||'D1 verileri yüklenemedi.')})
  ...
  const exportExcel=()=>{
    ...
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(data),'Kayıtlar');
    XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(summary),'Özet');
    XLSX.writeFile(wb,`Tur-Butcesi-${today}.xlsx`);
  };
  ...
  return <>
    ...
    <main className="main-dashboard">
      ...
      <section className="dashboard">
        <div className="kpis">
          <article><span>Toplam gelir</span><strong>{money(income,currency)}</strong><small>{active.filter(r=>INCOME_TYPES.has(r.type)).length} gelir kaydı</small></article>
          <article><span>Tur masrafı</span><strong>{money(expense,currency)}</strong><small>{active.filter(r=>r.type==='Tur Masrafı').length} gider kaydı</small></article>
          <article className="net"><span>Net kazanç</span><strong>{money(income-expense,currency)}</strong><small>Gelir eksi masraf</small></article>
          <article className="pending"><span>Alınmayı bekleyen</span><strong>{money(pending,currency)}</strong><small>{active.filter(r=>INCOME_TYPES.has(r.type)&&r.status==='Alınmadı').length} açık tahsilat</small></article>
        </div>
