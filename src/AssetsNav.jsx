...
function DepositPlaceholder(){
 const cards=[['Toplam Para','—','Mevduat ana para'],['Günlük Kazanç','—','Net günlük faiz'],['Günlük Getiri (%)','—','Net oran'],['Toplam Faiz','—','Vade getirisi'],['Ana Para','—','Mevduat tutarı'],['Faiz / Vade','—','Oran ve gün']];
 return <section className="asset-side deposit-side"><div className="asset-side-head"><div><span className="eyebrow">MEVDUAT FAİZİ</span><h2>Vadeli Mevduat</h2><p>Banka mevduatı ve günlük faiz takibi.</p></div><div className="deposit-status"><i/>BİLGİ BEKLENİYOR</div></div><section className="fund-kpis compact-side-kpis">{cards.map(([label,value,sub])=><article className="fund-kpi deposit-kpi" key={label}><span>{label}</span><strong>{value}</strong><small>{sub}</small></article>)}</section><article className="deposit-chart-placeholder"><div><span className="eyebrow">MEVDUAT GETİRİ GRAFİĞİ</span><h3>Faiz verileri burada gösterilecek</h3><p>Ana para, banka, faiz oranı ve vade bilgilerini verdiğinde günlük kazanç ile vade sonu getiriyi burada çizeceğiz.</p></div></article><article className="compact-details-card deposit-details"><div className="fund-card-title"><span className="eyebrow">MEVDUAT DETAYI</span><h3>Hazır</h3></div><dl className="compact-details-grid"><div><dt>Banka</dt><dd>—</dd></div><div><dt>Ana para</dt><dd>—</dd></div><div><dt>Brüt oran</dt><dd>—</dd></div><div><dt>Net oran</dt><dd>—</dd></div><div><dt>Vade</dt><dd>—</dd></div><div><dt>Stopaj</dt><dd>—</dd></div></dl></article></section>;
}
