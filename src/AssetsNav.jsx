...
function DepositChart(){
 ...
 const points=history.map((row,i)=>{const x=padX+(i/(history.length-1))*(width-padX*2),y=padY+((max-row.total)/range)*(height-padY*2);return{x,y,row}}),line=points.map((p,i)=>`${i?'L':'M'} ${p.x.toFixed(2)} ${p.y.toFixed(2)}`).join(' '),last=points.at(-1);
 return <div className="fund-chart-wrap"><div className="fund-chart-meta"><div><span>32 Günlük Net Birikim</span><strong>{tl(DEPOSIT.netEnd)}</strong></div><div className="fund-chart-stats"><span>Net faiz <b>{tl(netInterest)}</b></span><span>Brüt faiz <b>{tl(grossInterest)}</b></span></div></div><svg className="fund-chart deposit-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Akbank 32 günlük mevduat net birikim grafiği"><line x1={padX} x2={width-padX} y1={height/2} y2={height/2} className="fund-grid-line"/><line x1={padX} x2={width-padX} y1={padY} y2={padY} className="fund-grid-line"/><line x1={padX} x2={width-padX} y1={height-padY} y2={height-padY} className="fund-grid-line"/><path d={line} className="fund-line"/><circle cx={last.x} cy={last.y} r="5" className="fund-last-point"/>{points.map((p,i)=><circle key={p.day} cx={p.x} cy={p.y} r="9" className="fund-hit"><title>{`${dateText(p.row.date)} · ${tl(p.row.total)}`}</title></circle>)}</svg><div className="fund-chart-dates"><span>{dateText(DEPOSIT.startDate)}</span><span>{dateText(DEPOSIT.endDate)}</span></div></div>;
}
