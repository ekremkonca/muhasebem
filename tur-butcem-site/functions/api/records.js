...
const selectFields = 'id,date,tour,guest,agency,ship,type,amount,currency,status,note,deleted_at,created_at,updated_at';
  ...
  try {
    ...
    const statements = records.map((r) => db.prepare(`
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        type=excluded.type,amount=excluded.amount,currency=excluded.currency,status=excluded.status,note=excluded.note,
        deleted_at=NULL,updated_at=CURRENT_TIMESTAMP
    `).bind(r.id,r.date,r.tour,r.guest,r.agency,r.ship,r.type,r.amount,r.currency,r.status,r.note));
...
export async function onRequestPatch(context) {
  try {
    ...
    if (body?.record) {
      ...
      const result = await db.prepare(`UPDATE records SET date=?,tour=?,guest=?,agency=?,ship=?,type=?,amount=?,currency=?,status=?,note=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND deleted_at IS NULL`)
        .bind(r.date,r.tour,r.guest,r.agency,r.ship,r.type,r.amount,r.currency,r.status,r.note,r.id).run();
      if (!result.meta?.changes) return json({ error: 'Kayıt bulunamadı.' }, 404);
    ...
    const before = await db.prepare(`SELECT ${selectFields} FROM records WHERE id=?`).bind(id).first();
    const result = await db.prepare('UPDATE records SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND deleted_at IS NULL').bind(status,id).run();
    if (!result.meta?.changes) return json({ error: 'Kayıt bulunamadı.' }, 404);
