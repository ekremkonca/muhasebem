async function parseResponse(response) {
  ...
  if (!contentType.includes("application/json")) {
    const error = new Error(
      `API yanıtı geçersiz (${response.status}). Cloudflare Pages Functions deployment kontrol edilmeli.`,
    );
...
const setAuthCache = (data) => {
  if (data && typeof data === "object") {
    authCache = {
      ...
      configured:
        data.configured ??
        (data.authenticated ? true : (authCache?.configured ?? false)),
    };
  } else authCache = null;
...
export const getAuthState = (fresh = false) => {
  if (!fresh && authCache && Date.now() - authCacheAt < AUTH_CACHE_MS)
    return Promise.resolve(authCache);
  if (!fresh && authInFlight) return authInFlight;
  authInFlight = request("/api/auth")
    .then(setAuthCache)
    .finally(() => {
      authInFlight = null;
    });
...
export const deleteRecord = (id) =>
  request(`/api/records?id=${encodeURIComponent(id)}`, { method: "DELETE" });
export const permanentDeleteRecord = (id) =>
