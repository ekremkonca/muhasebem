...
const goHome=()=>{history.pushState(null,'',`${location.pathname}${location.search}`);window.dispatchEvent(new Event('popstate'));window.dispatchEvent(new Event('assetsviewchange'));window.scrollTo({top:0,behavior:'auto'})};
...
export default function HomeBrandNav(){
 ...
 useEffect(()=>{const sync=()=>setAssets(isAssets());sync();const observer=new MutationObserver(sync);observer.observe(document.body,{childList:true,subtree:true});window.addEventListener('hashchange',sync);window.addEventListener('popstate',sync);window.addEventListener('assetsviewchange',sync);return()=>{observer.disconnect();window.removeEventListener('hashchange',sync);window.removeEventListener('popstate',sync);window.removeEventListener('assetsviewchange',sync)}},[]);
