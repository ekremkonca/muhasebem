...
export default function ExpensesPage(){
 ...
 return <HomePage contentClassName="expenses-page"><section className="expenses-shell">
  ...
  <section className="expense-budget-card"><div><span className="eyebrow">AYLIK BÜTÇE</span><strong>{budget?money(budget):"Bütçe belirlenmedi"}</strong><small>{budget?money(Math.max(0,budget-total))+" kaldı":"Bir hedef gir"}</small></div><label>Bütçe<input type="number" min="0" step="100" value={budget||""} onChange={e=>setBudget(Number(e.target.value))} placeholder="0 ₺"/></label><div className="budget-track"><i style={{width:Math.min(100,used)+"%"}}/><span>{budget?"%"+used.toFixed(1):"%0"}</span></div></section>
