...
function V8Enhancements({ rows, income, expense, pending, currency }) {
  ...
  return <section className="v8-enhancements" aria-label="V8 finans araçları">
    ...
    <div className="v8-enhance-grid">
      {!hidden.includes("score") && <article className="v8-score-card"><button className="v8-info-button" onClick={()=>setDetail(true)} aria-label="Skor detayları">i</button><span>Kişisel finans skoru</span><strong>{score}<small>/100</small></strong><div className="v8-score-ring" style={{"--score":`${score * 3.6}deg`}}/><p>{score >= 80 ? "Finansal durumun çok iyi." : score >= 60 ? "Dengeli gidiyorsun." : "Bütçeni biraz daha yakından takip et."}</p></article>}
