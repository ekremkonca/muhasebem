...
function V8Enhancements({ rows, income, expense, pending, currency, tourCount }) {
  ...
  return <section className="v8-enhancements" aria-label="V8 finans araçları">
    ...
    <div className="v8-suite-grid">
      ...
      <article><div className="v8-card-title"><span>Yaklaşan ödemeler</span><small>{notices.length}</small></div>{notices.map(r=><div className="v8-debt" key={r.id}><span>{r.note || r.agency || r.tour || "Ödeme"}</span><b>{money(r.amount,currency)}</b></div>)}{!notices.length&&<p className="v8-muted">Yaklaşan ödeme yok.</p>}</article>
