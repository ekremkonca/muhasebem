...
function V8Enhancements({ rows, income, expense, pending, currency }) {
  ...
  const [unread, setUnread] = useState(() => { try { return Number(localStorage.getItem("v8-unread") || notices.length); } catch { return notices.length; } });
  ...
  return <section className="v8-enhancements" aria-label="V8 finans araçları">
    ...
    <div className="v8-enhance-grid">{cardOrder.map((id) => id === "score" ? (!hidden.includes(id) && <article draggable className="v8-score-card v8-draggable-card" onDragStart={()=>setDragCard(id)} onDragOver={e=>e.preventDefault()} onDrop={()=>moveCard(id)} key={id}><button className="v8-info-button" onClick={()=>setDetail(true)} aria-label="Skor detayları">i</button><span>Kişisel finans skoru</span><strong>{score}<small>/100</small></strong><div className="v8-score-ring" style={{"--score":`${score * 3.6}deg`}}/><p>{score >= 80 ? "Finansal durumun çok iyi." : score >= 60 ? "Dengeli gidiyorsun." : "Bütçeni biraz daha yakından takip et."}</p></article>) : id === "chart" ? (!hidden.includes(id) && <article draggable className="v8-chart-card v8-draggable-card" onDragStart={()=>setDragCard(id)} onDragOver={e=>e.preventDefault()} onDrop={()=>moveCard(id)} key={id}><div className="v8-card-title"><span>Kategori dağılımı</span><small>{currency}</small></div><div className="v8-bars">{cats.map((cat) => <div className="v8-bar-row" key={cat.type}><span>{cat.type.replace("Tur ", "")}</span><div><i style={{width:`${Math.max(4,cat.value/max*100)}%`}}/></div><b>{money(cat.value, currency)}</b></div>)}</div></article>) : (!hidden.includes(id) && <article draggable className="v8-notice-card v8-draggable-card" onDragStart={()=>setDragCard(id)} onDragOver={e=>e.preventDefault()} onDrop={()=>moveCard(id)} key={id}><div className="v8-card-title"><span>Bildirim merkezi</span><small>{notices.length} bekleyen</small></div>{notices.length ? notices.map((r) => <div className="v8-notice" key={r.id}><i/><span>{r.tour || "Kayıt"}<small>{r.due_date ? `Vade: ${r.due_date}` : "Ödeme bekliyor"}</small></span></div>) : <p>Yeni bildirimin yok.</p>}</article>)}</div>
    <div className="v8-suite-grid">
      <article><div className="v8-card-title"><span>Gelir / gider trendi</span><small>6 ay</small></div><div className="v8-line-chart">{monthBars.map((m,i)=><div key={i}><i style={{height:`${Math.max(8,Math.min(100,m.value/(Math.max(...monthBars.map(x=>x.value),1))*100))}%`}}/><small>{m.label}</small></div>)}</div></article>
      ...
      <article><div className="v8-card-title"><span>Yaklaşan ödemeler</span><small>{notices.length}</small></div>{notices.map(r=><div className="v8-debt" key={r.id}><span>{r.tour||"Ödeme"}</span><b>{money(r.amount,currency)}</b></div>)}{!notices.length&&<p className="v8-muted">Yaklaşan ödeme yok.</p>}</article>
      <article><div className="v8-card-title"><span>Bildirim merkezi</span><button className="v8-link" onClick={()=>{setUnread(0);localStorage.setItem("v8-unread","0")}}>{unread ? `${unread} okunmadı` : "Tümü okundu"}</button></div><p className="v8-muted">Vade, hedef ve haftalık özet bildirimleri burada görünecek.</p></article>
    </div>
