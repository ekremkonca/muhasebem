...
function V8Enhancements({ rows, income, expense, pending, currency, tourCount, net, convert }) {
  ...
  const settled = rows.filter((r) => r.status === "Ödendi");
  const categoryBreakdown = [
    ["Tur Geliri (TL)", (r) => r.type === "Tur Geliri" && r.currency === "TRY"],
    ["Bahşiş (TL)", (r) => r.type === "Bahşiş" && r.currency === "TRY"],
    ["Komisyon (TL)", (r) => r.type === "Komisyon" && r.currency === "TRY"],
    ["Bahşiş (Döviz)", (r) => r.type === "Bahşiş" && r.currency !== "TRY"],
    ["Komisyon (Döviz)", (r) => r.type === "Komisyon" && r.currency !== "TRY"],
    ["Masraf", (r) => r.type === "Tur Masrafı"],
  ];
  const cats = categoryBreakdown.map(([type, matches]) => ({
    type,
    value: settled.filter(matches).reduce((sum, row) => sum + convert(row), 0),
