...
function PaletteIcon(){return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3a9 9 0 1 0 0 18h1.2a2 2 0 0 0 1.5-3.3 1.9 1.9 0 0 1 1.4-3.2H18a3 3 0 0 0 3-3A8.5 8.5 0 0 0 12 3z"/><circle cx="7.5" cy="10" r=".8" fill="currentColor" stroke="none"/><circle cx="10" cy="6.8" r=".8" fill="currentColor" stroke="none"/><circle cx="14.2" cy="6.6" r=".8" fill="currentColor" stroke="none"/></svg>}
...
export default function ThemeSwitcher(){
 ...
 return <div className="theme-menu v8-theme-menu" ref={ref}>
  ...
  {open&&<div id={panelId} className="theme-panel v8-theme-panel theme-preview-panel" role="group" aria-label="Tema renkleri">{THEMES.map(([id,name,color])=><button key={id} type="button" className={theme===id?'active':''} style={{'--preview-color':color}} aria-pressed={theme===id} onClick={()=>{setTheme(id);setOpen(false);triggerRef.current?.focus()}}><i style={{'--theme-dot':color}}/><span>{name}<small>Önizleme</small></span></button>)}</div>}
 </div>
}
