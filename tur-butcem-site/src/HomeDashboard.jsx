...
export default function HomeDashboard(){
 ...
 const visible=useMemo(()=>news.filter(item=>filter==='all'||item.region===filter).slice(0,24),[news,filter]);
 ...
 return <main className="home-market-dashboard">
  ...
  <div className="market-dashboard-grid">
   <section className="news-panel"><header className="news-panel-head"><div><span className="section-eyebrow">SON DAKİKA</span><h2>Ekonomi Haber Akışı</h2><small>{updatedAt?`Son kontrol ${relativeTime(updatedAt)}`:'Akış hazırlanıyor'}</small></div><button type="button" onClick={refresh} disabled={loading} aria-label="Haberleri yenile">{loading?'Yükleniyor…':'↻ Yenile'}</button></header>
    <div className="news-filters" role="tablist" aria-label="Haber bölgesi"><button className={filter==='all'?'active':''} onClick={()=>setFilter('all')}>Tümü</button><button className={filter==='tr'?'active':''} onClick={()=>setFilter('tr')}>Türkiye</button><button className={filter==='world'?'active':''} onClick={()=>setFilter('world')}>Dünya</button></div>
    {error&&<div className="news-error">{error} Otomatik olarak tekrar denenecek.</div>}
    <div className="breaking-news-list">{visible.map((item,index)=><a href={item.url} target="_blank" rel="noreferrer" className="breaking-news-row" key={item.id}><span className={`news-rank ${index<3?'hot':''}`}>{index<3?'SON':'•'}</span><span className="news-copy"><strong>{item.title}</strong><small><b>{item.source}</b><time dateTime={item.publishedAt}>{relativeTime(item.publishedAt)}</time></small></span><span className="news-open">↗</span></a>)}{!loading&&!visible.length&&<div className="news-empty">Şu anda bu bölümde yeni haber bulunmuyor.</div>}{loading&&!news.length&&Array.from({length:8},(_,i)=><div className="news-skeleton" key={i}/>)}</div>
   </section>
   <aside className="market-sidebar"><header><div><span className="section-eyebrow">TRADINGVIEW</span><h2>Canlı Piyasalar</h2></div><span className="market-live"><i/> CANLI</span></header><div className="market-overview-host"><TradingViewWidget script="embed-widget-market-overview.js" config={overviewConfig}/></div><p className="market-disclaimer">Veriler TradingView tarafından sağlanır. Bazı borsalarda veriler gecikmeli olabilir.</p></aside>
  </div>
 </main>;
}
