...
function TradingViewWidget({script,config,className=''}){
 ...
 useEffect(()=>{
  ...
  let cancelled=false;
  const timer=setTimeout(()=>{if(cancelled)return;
   ...
   const loader=document.createElement('script');loader.src=`https://s3.tradingview.com/external-embedding/${script}`;loader.async=true;loader.type='text/javascript';loader.textContent=JSON.stringify(config);container.appendChild(loader);node.replaceChildren(container);
  },0);
...
export default function HomeDashboard(){
 ...
 const visible=useMemo(()=>news.filter(item=>filter==='all'?item.region!=='forum':item.region===filter).slice(0,24),[news,filter]);
 const sourceLinks=[
  {name:'Watcher.Guru',handle:'@WatcherGuru',url:'https://x.com/WatcherGuru',tone:'watcher'},
  {name:'The Spectator Index',handle:'@spectatorindex',url:'https://x.com/spectatorindex',tone:'spectator'},
  {name:'Current Report',handle:'@Currentreport1',url:'https://x.com/Currentreport1',tone:'current'},
 ];
 ...
 return <main className="home-market-dashboard">
  <section className="market-hero"><div className="market-welcome"><div><span className="live-kicker"><i/> CANLI PİYASA MASASI</span><h1>Dünyayı ve piyasaları<br/><em>tek bakışta yakala.</em></h1><p>Gürültüden arındırılmış son dakika akışı, bağımsız kaynaklar ve canlı piyasa verileri.</p></div><div className="market-clock"><span>İstanbul · canlı</span><strong>{clockTime(now)}</strong><small>{new Intl.DateTimeFormat('tr-TR',{weekday:'long',day:'2-digit',month:'long'}).format(now)}</small></div></div>
   <div className="source-strip"><span className="source-strip-label">TAKİPTEKİ KAYNAKLAR</span>{sourceLinks.map(source=><a key={source.name} href={source.url} target="_blank" rel="noreferrer" className={`source-pill ${source.tone}`}><i>{source.name.charAt(0)}</i><span><strong>{source.name}</strong><small>{source.handle}</small></span><b>↗</b></a>)}</div>
  </section>
  ...
  <div className="market-dashboard-grid">
   <section className="news-panel"><header className="news-panel-head"><div><span className="section-eyebrow">HABER MERKEZİ</span><h2>Gündem akışı</h2><small>{updatedAt?`Son kontrol ${relativeTime(updatedAt)}`:'Akış hazırlanıyor'}</small></div><button type="button" onClick={refresh} disabled={loading} aria-label="Haberleri yenile">{loading?'Yükleniyor…':'↻ Yenile'}</button></header>
    ...
    <div className="breaking-news-list">{visible.map((item,index)=><a href={item.url} target="_blank" rel="noreferrer" className="breaking-news-row" key={item.id}><span className={`news-rank ${item.region==='independent'?'independent':index<3?'hot':''}`}>{item.region==='independent'?'X':index<3?'SON':'•'}</span><span className="news-copy"><strong>{item.title}</strong><small><b>{item.source}</b><time dateTime={item.publishedAt}>{relativeTime(item.publishedAt)}</time></small></span><span className="news-open">↗</span></a>)}{!loading&&!visible.length&&<div className="news-empty">Bu kaynakta henüz yeni bir paylaşım yok. Birazdan yeniden kontrol edilecek.</div>}{loading&&!news.length&&Array.from({length:7},(_,i)=><div className="news-skeleton" key={i}/>)}</div>
   </section>
   <aside className="market-sidebar"><header><div><span className="section-eyebrow">PİYASA RADARI</span><h2>Canlı piyasalar</h2></div><span className="market-live"><i/> CANLI</span></header><div className="market-overview-host"><TradingViewWidget script="embed-widget-market-overview.js" config={overviewConfig}/></div><a className="bist-tradingview-link" href="https://tr.tradingview.com/symbols/BIST-XU100/" target="_blank" rel="noreferrer"><span><b>Detaylı grafikleri incele</b><small>TradingView üzerinde tüm piyasa verileri</small></span><strong>Grafiğe git ↗</strong></a><p className="market-disclaimer">Piyasa verileri TradingView tarafından sağlanır ve gecikmeli olabilir. Yatırım tavsiyesi değildir.</p></aside>
  </div>
 </main>;
}
